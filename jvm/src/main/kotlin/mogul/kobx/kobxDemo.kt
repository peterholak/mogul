...
class KobxCounter(kobx: KobX) : ObserverComponent<Unit>(kobx) {

    var count by kobx.observable(0)

    override fun renderObserved(): Element {
        return gui {
            layoutBox {
 ...
 */
val globalKobx = KobX()
val kobxCounterType = ElementType("KobxCounter", { KobxCounter(globalKobx) })
fun GuiBuilder.kobxCounter() {
    children.add(Element(kobxCounterType, Unit))
}

class GlobalKobxCounter(kobx: KobX) : ObserverComponent<GlobalKobxCounter.Props>(kobx) {
    /**
...
val globalKobxCounterType = ElementType("GlobalKobxCounter", { GlobalKobxCounter(globalKobx) })
